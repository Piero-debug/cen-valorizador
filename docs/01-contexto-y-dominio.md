...
| **Instalación**            | Unidad física valorizada (una subestación específica, una línea específica). Una obra puede contener instalaciones de tipos distintos.                                                                                                                                                                                                        |
| **Componente tipificado**   | Módulo estandarizado del catálogo CEN (paño, barra, tramo de línea, transformador, equipo de compensación). Trae una composición predefinida de partidas.                                                                                                                                                                                    |
| **Paño**                   | Conjunto de equipos eléctricos que permite la conexión de líneas, transformador, compensación y maniobras operaciones de barra<br />Compuesto por equipos como: Interruptor, transformador de corriente, transformador de voltaje, pararrayo, desconectadores (con y sin puesta a tierra), aisladores de pedestal, trampa de onda              |
| Barra                             | Nodo físico de conexión paños, en tecnología AIS o GIS, con diferentes configuraciones que dan flexibilidad operacional, tales como:<br />Barra Simple, Barra Simple Seccionada, Barra Principal Seccionada y Barra de Transferencia, Barra Principal y Barra de Transferencia, Doble Barra y Barra de Transferencia y Interruptor y Medio |
| Línea                            | Conjunto de torres y conductores que permiten conectar dos subestaciones, se compone de:<br />torres (suspensión, anclaje, remate), conductores, asiladores, herraje                                                                                                                                                                              |
| Transformador                     | Equipo eléctrico que permite transformar diferentes niveles de tensión                                                                                                                                                                                                                                                                           |
...
| **Diagrama Grafo (DG)**     | Representación esquemática del sistema eléctrico que el editor canvas construye. Modo principal de visualización del prototipo.                                                                                                                                                                                                                |
| **Diagrama Unilineal (DU)** | Representación esquemática del sistema eléctrico a desarrollar como vista futura, no implementada en el prototipo                                                                                                                                                                                                                               |
...
| **HIS**                     | Tecnología híbrida (barra AIS + equipo GIS).                                                                                                                                                                                                                                                                                                     |
| **Tipos de interruptor**    | Solo en subestaciones tipo AIS, se dan tres posibles tipo de tecnología de interruptor:<br />1) Tanque vivo (solo el equipo interruptor) <br />2) Tanque muerto (equipo interruptor + TC) <br />3) hibrido GIS compacto (interruptor + TC + desconectador) en versión trifásico y monopolar                                                    |
...
│   ┌─────────────────────────────────────────────────┐   │
│   │ Instalación (1..N)                              │   │
│   │   (subestación específica / línea)              │   │
│   │                                                 │   │
│   │   ┌─────────────────────────────────────────┐   │   │
│   │   │ Componente tipificado (1..N)            │   │   │
│   │   │   (paño, barra, tramo línea, trafo, …)  │   │   │
│   │   │                                         │   │   │
│   │   │   ┌─────────────────────────────────┐   │   │   │
...
| Configuración | (depende del decreto: simple, interruptor y medio, etc. (ver glosario)                                                  |
| Posiciones     | Entero. Define cuántos paños caben en la barra.                                                                       |
| Capacidad      | Definida en el decreto (valor en MVA). Determina internamente el tipo de conductor de barra (sin elección del usuario) |
...
| Configuración | según descripción decreto, tipicamente barra simple, barra simple seccionada, Barra Principal Seccionada y Barra de Transferencia |
| Posiciones     | Entero                                                                                                                               |
...
El paño son el conjunto de equipos que ocupa una posición en una barra que conecta a un elemento aguas abajo (línea, trafo, compensador) o entre barras (acoplador, seccionador).

#### 5.4.1 Paño (estándar)

| Atributo            | Valores                                                                                                     |
| ------------------- | ----------------------------------------------------------------------------------------------------------- |
| Tipo de interruptor | Tres tipos solo en barra tipo AIS (ver glosario)                                                            |
...
| Tecnología | Opciones                                                                  |
| ----------- | ------------------------------------------------------------------------- |
| AIS         | `tanque vivo`, `tanque muerto`, `híbrido`, `híbrido monopolar` |
...
| Tipo de torre                           | `reticulada`, `poste hormigón`, `monoposte`, `subterránea`   |
...
| ----------------- | :----------------------------: | :----------------------------: | :-----------------------: | :------------------: |
| Barra AT          |               ●               |    ● (extensión y nueva)    |            –            |          –          |
| Barra MT          |               ●               |               ●               |            –            |          –          |
| Paño (estándar) |               ●               |               ●               |     ● (en extremos)     | ● (cambio equipos) |
| Paño Diagonal    |               ●               |               ●               |     ● (en extremos)     | ● (cambio equipos) |
| Línea            | ● (enlace de seccionamiento) |               –               |   ● (tramo principal)   |          ●          |
| Transformador     |               ●               |               ●               |            –            |          –          |
| Compensación     | ● (BBCC, compensación barra) | ● (BBCC, compensación barra) | ● (compensación línea) |          –          |
...
Instalación (hereda zona, plazo)
   ↓
Barra (define tecnología, tensión, configuración, posiciones (barra extendida y terreno nivelado))
   ↓
Paño (hereda tecnología de la barra, tensión, configuración → se elige opciones de interruptor)
   ↓
